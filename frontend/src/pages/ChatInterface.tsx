...
export function ChatInterface() {
  ...
  // Refs for auto-scroll and stopping generation
  ...
  return (
    <div className="flex h-screen bg-black overflow-hidden font-sans text-white">
      ...
      <div className="flex-1 flex flex-col relative w-full h-full border-l border-white/5">
        {/* Header - Merged UI */}
        <div className="h-14 flex items-center justify-between px-4 bg-black/50 backdrop-blur-md z-20 border-b border-white/5">
        ...
        <div className="flex-1 overflow-y-auto scroll-smooth relative custom-scrollbar">
          {isHistoryLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full px-4 text-center">
              ...
              <h1 className="text-3xl font-bold text-white mb-3 animate-in fade-in slide-in-from-bottom-3 duration-700">
              ...
              <div className="w-full max-w-2xl relative group animate-in slide-in-from-bottom-6 duration-700 delay-200">
                ...
                <div className="relative bg-[#0a0a0a] rounded-2xl border border-white/10 shadow-2xl overflow-hidden">
                  <textarea 
                    ...
                    className="w-full bg-transparent border-none outline-none text-white placeholder-text-secondary/40 px-4 py-4 resize-none h-[56px] max-h-[200px]"
                    rows={1}
                  />
                  <div className="flex justify-between items-center px-3 pb-3">
                    ...
                    {isLoading ? (
                      <button onClick={handleStop} className="p-2.5 rounded-xl bg-white/10 text-white hover:bg-white/20 transition-all flex items-center gap-2 border border-white/10">
                        <Square size={14} fill="white" />
                        <span className="text-[10px] font-bold uppercase tracking-wider">Stop</span>
                      </button>
                    ) : (
                      <button onClick={() => handleSend()} disabled={!input.trim()} className="p-2.5 rounded-xl bg-primary text-black font-bold">
                        <Send size={18} />
                      </button>
                    )}
                  </div>
        ...
        {/* Floating Input Area (Appears only during active chat) */}
        {messages.length > 0 && (
                  ...
                  <div className="flex items-center gap-2">
                    {isLoading ? (
                      ...
                      <button 
                        onClick={() => handleSend()}
                        disabled={!input.trim()}
                        className="p-2.5 rounded-xl bg-primary text-black font-bold hover:scale-105 transition-all disabled:opacity-20"
                      >
                        <Send size={18} />
                      </button>
                    )}
