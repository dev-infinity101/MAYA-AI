...
import { useCallback, useState, type ChangeEvent, type FormEvent, type SVGProps } from "react";
      ...
      {providers.map((provider) => {
        ...
        return (
          <Button
            ...
            type="button"
            variant="outline">
            <Icon aria-hidden="true" className="size-4" />Continue with {provider.name}
          </Button>
        );
      })}
    </div>
...
export default function AuthLoginForm({
  ...
  const [localErrors, setLocalErrors] = useState<LoginErrors>({});
      ...
      <CardContent>
        <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
          ...
          {showSocialLogin && socialProviders.length > 0 && (
            <>
              <SocialLoginButtons onSocialLogin={onSocialLogin} providers={socialProviders} />
              <EmailSeparator />
            </>
          )}
